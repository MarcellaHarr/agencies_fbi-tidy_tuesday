...
dfPlot <- df_sf %>%
  ...
  mutate(coords = map_if(geometry, 
         ...
         absent_coords = !is.na(lat) & !is.na(lon) & 
           ...
           log_agency_count = log1p(agency_count)
         )
...
ggplot() + 
  ...
  labs(title = "Agency Types and Counts in Newark, NJ", 
       subtitle = "Street overlays and log-scaled agency density") + 
  coord_sf(xlim = c(-74.27, -74.11), 
           ylim = c(40.66, 40.80), 
           expand = FALSE) +
